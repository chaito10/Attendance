...
|-----|--------------|---------|
| Apple Silicon (M1/M2/M3/M4) | arm64 | `attendance-v0.2.0-macos-arm64.tar.gz` |
| Intel | x86_64 | `attendance-v0.2.0-macos-x86_64.tar.gz` |
...
1. Download the archive for your architecture from the
   ...
   ```bash
   curl -L -o attendance-v0.2.0-macos-arm64.tar.gz \
     https://github.com/chaito10/Attendance/releases/download/v0.2.0/attendance-v0.2.0-macos-arm64.tar.gz
   ```

2. Extract it and make it executable:

   ```bash
   tar xzf attendance-v0.2.0-macos-arm64.tar.gz
   chmod +x attendance
   ```
